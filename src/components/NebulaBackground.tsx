...
const NebulaBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      // ══════ SPIRAL GALAXY (distant, top-right) ══════
      ctx.save();
      ...
      // Red dwarf (bottom center-left)
      const p5x = w * 0.35, p5y = h * 0.88, p5r = w * 0.025;
      const p5grad = ctx.createRadialGradient(p5x - p5r * 0.2, p5y - p5r * 0.2, p5r * 0.05, p5x, p5y, p5r);
      p5grad.addColorStop(0, 'rgba(255,100,60,0.6)');
      p5grad.addColorStop(0.5, 'rgba(180,50,20,0.35)');
      p5grad.addColorStop(1, 'rgba(100,20,5,0.15)');
      ctx.beginPath(); ctx.arc(p5x, p5y, p5r, 0, Math.PI * 2); ctx.fillStyle = p5grad; ctx.fill();
